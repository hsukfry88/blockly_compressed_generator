...
Blockly.BlockSvg.prototype.showContextMenu_ = function (e) {
  ...
  if (this.isDeletable() && this.isMovable() && !block.isInFlyout) {
    ...
    var until_duplicateOption = {
      ...
      enabled: true,
      callback: function () {
        Blockly.until_duplicate_(block);
      }
    };
    if (this.getDescendants().length > this.workspace.remainingCapacity()) {
      until_duplicateOption.enabled = false;
    }
    menuOptions.push(until_duplicateOption);
...
Blockly.BlockSvg.prototype.updateColour = function () {
  if (this.disabled && this.unusabled) {
...
/**
 * Usable or Unusable a block.
 */
Blockly.BlockSvg.prototype.updateUnusabled = function () {
  var hasClass = Blockly.hasClass_(/** @type {!Element} */ (this.svgGroup_), 'blocklyUnusabled');
  if (this.unusabled) {
    if (!hasClass) {
      Blockly.addClass_(/** @type {!Element} */ (this.svgGroup_), 'blocklyUnusabled');
    }
  } else {
    if (hasClass) {
      Blockly.removeClass_(/** @type {!Element} */ (this.svgGroup_), 'blocklyUnusabled');
    }
  }
  var children = this.getChildren();
  for (var i = 0, child; child = children[i]; i++) {
    child.updateUnusabled();
  }
};
...
/**
 * Set whether the block is unusabled or not.
 * @param {boolean} unusabled True if usabeld.
 */
Blockly.BlockSvg.prototype.setUnusabled = function (unusabled) {
  if (this.unusabled != unusabled) {
    Blockly.BlockSvg.superClass_.setUnusabled.call(this, unusabled);
    if (this.rendered) {
      this.updateUnusabled();
    }
  }
};
